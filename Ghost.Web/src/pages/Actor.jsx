...
const fetchRandomVideo = (id) => async (params) => {
  const videoResult = await axios.get(
    `/media/actor/${encodeURIComponent(id)}/random?${constructVideoParams(
      params
    )}`
  );
...
export const Actor = () => {
  ...
  const handleToggleFavourite = async () =>
    (await axios.put(`/user/actor/${actorsPage.id}`)).data;
  ...
  return (
    <>
      ...
      <VideoView
        fetchFn={fetchVideos(params.id)}
        fetchRandomVideoFn={fetchRandomVideo(params.id)}
      />
    </>
  );
};
